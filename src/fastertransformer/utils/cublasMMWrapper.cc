...
#include <iostream>
// #define PRINT_GEMM_INFO
#include "ladder_gemm/ladder_kernel.h"
...
bool call_ladder_gemm_kernel(
    ...
    if (!GlobalConfig::instance().ladder_4bit) {
        return false;
    }
    if(dtype != CUDA_R_16F) {
        // std::cout << "call ladder kernel failed, unmatched data type." << std::endl;
        return false;
    }
    try {
        // ret = ladder_gemm_fp16xfp16_fp16((half*)B, (half*)A, (half*)C, n, m, k, 0, 1);
        int ret = ladder_gemm_fp16xnf4_fp16((half*)B, (half*)A, (half*)C, n, m, k, 0, 1);
        if(ret != 0) {
            // std::cout << "call ladder kernel failed, return value -1, shape (m, n, k) = (" << m << ", " << n << ", " << k << "), transa = " << transa << ", transb = " << transb << std::endl;
            return false;
        }
        sync_check_cuda_error();
    }
    catch(...) {
        // std::cout << "call ladder kernel failed, error throwed." << std::endl;
        return false;
    }
    // std::cout << "ladder gemm kernel is used." << std::endl;
    return true;
}
}
...
{
    FT_LOG_DEBUG(__PRETTY_FUNCTION__);
#ifdef PRINT_GEMM_INFO
    FT_LOG_INFO(__PRETTY_FUNCTION__);
    std::cout << "transa = " << transa << ", transb = " << transb << ", m = " << m << ", n = " << n << ", k = " << k 
              << ", Atype = " << Atype << ", lda = " << lda << ", Btype = " << Btype << ", ldb = " << ldb << ", Ctype = " 
              << Ctype << ", ldc = " << ldc << ", computeType = " << computeType << std::endl;
#endif
    mu_->lock();
    if(!call_ladder_gemm_kernel(transa, transb, A, B, C, m, n, k, Atype))
                           ...
                           float             f_beta)
{
    ...
    FT_LOG_INFO(__PRETTY_FUNCTION__);
    std::cout << "transa = " << transa << ", transb = " << transb << ", m = " << m << ", n = " << n << ", k = " << k 
              << ", Atype = " << Atype_ << ", lda = " << lda << ", Btype = " << Btype_ << ", ldb = " << ldb << ", Ctype = " 
              << Ctype_ << ", ldc = " << ldc << ", computeType = " << computeType_ << std::endl;
#endif
    ...
    if(!call_ladder_gemm_kernel(transa, transb, A, B, C, m, n, k, Atype_))
    if (using_cublasLt) {
                           ...
                           const int         ldc)
{
    ...
    std::cout << "transa = " << transa << ", transb = " << transb << ", m = " << m << ", n = " << n << ", k = " << k 
              ...
              << Ctype << ", ldc = " << ldc << ", computeType = " << computeType << std::endl;
#endif
    if(call_ladder_gemm_kernel(transa, transb, A, B, C, m, n, k, Atype)) return;
    cublasLtMatmulDesc_t   operationDesc = NULL;
    cublasLtMatrixLayout_t Adesc = NULL, Bdesc = NULL, Cdesc = NULL;
                                         ...
                                         const float       f_beta)
{
    ...
    FT_LOG_INFO(__PRETTY_FUNCTION__);
    std::cout << "transa = " << transa << ", transb = " << transb << ", m = " << m << ", n = " << n << ", k = " << k 
              << ", Atype = " << Atype_ << ", lda = " << lda << ", strideA = " << strideA << ", Btype = " << Btype_ 
              << ", ldb = " << ldb << ", strideB = " << strideB << ", Ctype = " << Ctype_ << ", ldc = " << ldc 
              << ", strideC = " << strideC << ", computeType = " << computeType_ << ", batch_count = " << batch_count << std::endl;
#endif
    half h_alpha = (half)f_alpha;
    half h_beta  = (half)f_beta;
                                         ...
                                         cudaDataType_t    computeType)
{
    ...
    FT_LOG_INFO(__PRETTY_FUNCTION__);
    std::cout << "transa = " << transa << ", transb = " << transb << ", m = " << m << ", n = " << n << ", k = " << k 
              << ", Atype = " << AType << ", lda = " << lda << ", strideA = " << strideA << ", Btype = " << BType 
              << ", ldb = " << ldb << ", strideB = " << strideB << ", Ctype = " << CType << ", ldc = " << ldc 
              << ", strideC = " << strideC << ", computeType = " << computeType << ", batch_count = " << batch_count << std::endl;
#endif
    half h_alpha = (half)f_alpha;
    half h_beta  = (half)f_beta;
                                  ...
                                  const int          batch_count)
{
    ...
    FT_LOG_INFO(__PRETTY_FUNCTION__);
    std::cout << "transa = " << transa << ", transb = " << transb << ", m = " << m << ", n = " << n << ", k = " << k 
              << ", Atype = " << Atype_ << ", lda = " << lda << ", Btype = " << Btype_ 
              << ", ldb = " << ldb << ", Ctype = " << Ctype_ << ", ldc = " << ldc 
              << ", computeType = " << computeType_ << ", batch_count = " << batch_count << std::endl;
#endif
