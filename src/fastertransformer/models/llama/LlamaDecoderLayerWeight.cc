...
template<typename T>
int get_q_size(int k, int n)
{
    int qweight_size = k * n / sizeof(T) / 2;
    int scales_size = k / GlobalConfig::instance().group_size * n;
    return qweight_size + scales_size;
};
...
void LlamaDecoderLayerWeight<T>::loadModel(std::string dir_path, FtCudaDataType model_file_type)
{
    ...
    if (GlobalConfig::instance().ladder_4bit) {
        loadWeightFromBin<T>(weights_ptr[2],
                         {get_q_size<T>(hidden_units_, 3 * hidden_units_) / tensor_para_size_},
                         dir_path + ".attention.query_key_value.weight." + rank_spec + ".bin",
                         model_file_type);

        loadWeightFromBin<T>(weights_ptr[4],
                         {get_q_size<T>(hidden_units_, hidden_units_) / tensor_para_size_},
                         dir_path + ".attention.dense.weight." + rank_spec + ".bin",
                         model_file_type);

        loadWeightFromBin<T>(weights_ptr[6],
                         {get_q_size<T>(hidden_units_, inter_size_) / tensor_para_size_},
                         dir_path + ".mlp.gate_proj.weight." + rank_spec + ".bin",
                         model_file_type);
        ...
        loadWeightFromBin<T>(weights_ptr[8],
                         ...
                         model_file_type);
        loadWeightFromBin<T>(weights_ptr[10],
                         {get_q_size<T>(inter_size_, hidden_units_) / tensor_para_size_},
                         dir_path + ".mlp.down_proj.weight." + rank_spec + ".bin",
                         model_file_type);
    }
    else if (int8_mode_ == 0) {        
        loadWeightFromBin<T>(weights_ptr[2],
...
void LlamaDecoderLayerWeight<T>::mallocWeights()
{
    ...
    if (GlobalConfig::instance().ladder_4bit) {
        deviceMalloc(&weights_ptr[2], get_q_size<T>(hidden_units_, 3 * hidden_units_) / tensor_para_size_);  // qkv weight
        deviceMalloc(&weights_ptr[4], get_q_size<T>(hidden_units_, hidden_units_) / tensor_para_size_);  // attention output weight
        deviceMalloc(&weights_ptr[6], get_q_size<T>(hidden_units_, inter_size_) / tensor_para_size_);   // intermediate_weight kernel
        deviceMalloc(&weights_ptr[8], get_q_size<T>(hidden_units_, inter_size_) / tensor_para_size_);  // intermediate_weight2 kernel
        deviceMalloc(&weights_ptr[10], get_q_size<T>(inter_size_, hidden_units_) / tensor_para_size_);  // output_weight kernel    
    } else if (int8_mode_ == 0) {
