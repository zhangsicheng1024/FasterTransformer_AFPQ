...
def split_and_convert(args):
    ...
    model_state_dict = torch.load(args.quant_file, map_location="cpu")
    ...
    for l in range(num_layers):
        ...
        qkv_weights = np.stack([
            param_to_weights(model_state_dict[f'model.layers.{l}.self_attn.q_proj.qweight']),
            param_to_weights(model_state_dict[f'model.layers.{l}.self_attn.k_proj.qweight']),
            param_to_weights(model_state_dict[f'model.layers.{l}.self_attn.v_proj.qweight']),
        ])
        qkv_weights = np.transpose(qkv_weights, (0, 2, 1))
        # workaround: flatten and concat
        # maynot work for multi-gpu
        qkv_weights = qkv_weights.flatten()
        qkv_scales = np.stack([
            param_to_weights(model_state_dict[f'model.layers.{l}.self_attn.q_proj.scales']),
            param_to_weights(model_state_dict[f'model.layers.{l}.self_attn.k_proj.scales']),
            param_to_weights(model_state_dict[f'model.layers.{l}.self_attn.v_proj.scales']),
        ])
        qkv_scales = np.transpose(qkv_scales, (1, 0, 2))
        qkv_scales = qkv_scales.flatten()
        qkv_weights = np.concatenate((qkv_weights, qkv_scales.view(np.int32)))
        ...
        def qlinear_weight(base_name):
            qweight = param_to_weights(model_state_dict[f'{base_name}.qweight']).T.flatten()
            scales = param_to_weights(model_state_dict[f'{base_name}.scales']).flatten()
            return np.concatenate((qweight, scales.view(np.int32)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-quant_file', '-i_q', type=str, help='filename of quantized state dict', required=True)
